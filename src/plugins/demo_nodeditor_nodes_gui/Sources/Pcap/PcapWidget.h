 ...
 * Connects directly to PcapEngine for packet/status signals (headless-compatible).
 */
...
public:
    ...
    /// Connect to engine for packet/status signals (called by NodeWidgetFactory)
    void setEngine(PcapEngine *engine);
...
private:
    ...
    QCheckBox *m_promiscCheck = nullptr;
