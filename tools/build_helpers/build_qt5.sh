#!/bin/bash
# Build Daqster with Qt5
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
QT_PREFIX="${QT_PREFIX:-$HOME/bin/Qt/5.15.2/gcc_64}"
BUILD_DIR="${BUILD_DIR:-build-qt5-checkpoint}"
CLEAN="${CLEAN:-0}"
VERBOSE="${VERBOSE:-0}"

usage() {
    cat <<EOF
Usage: build_qt5.sh [OPTIONS]
...
echo "=== Qt5 build completed successfully ==="
