...
- [ ] 1. **Lazy кешове.** `VideoFrameData` с lazy `asImage()` (QImage кеш) +
       GL текстура кеш.
...
- [ ] 3. **Fan-out.** N консуматора споделят една конверсия (не N×
       `toImage()`).
- [ ] 4. **Фаза 1.** `ImageData` остава; новите нодове работят редом без да
       чупят съществуващите графи.
- [ ] 5. **Фаза 2.** Еквивалентност доказана — ръчна оценка от потребителя
       (визуално; без пиксел-диф харнес).
- [ ] 6. **Фаза 3.** `VideoTransformNode`, `FrameToTensorNode`,
       `VideoOutputNode` fallback, saved графи мигрирани.
- [ ] 7. **Qt5 + Qt6 builds PASS.**
- [ ] 8. **Тестове** (отложени по стояща инструкция).
