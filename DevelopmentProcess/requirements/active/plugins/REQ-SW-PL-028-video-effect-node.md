...
- [x] 8. **Backend индикация в комбобокса:** комбобоксът показва backend-а на
       всеки ефект като суфикс — "(GPU)" за GpuOrCpu, "(CPU)" за CpuOnly
       (напр. "Brightness (GPU)", "Blur (CPU)"). Registry ключът остава
       spec.id — saved графите се зареждат непроменени.
