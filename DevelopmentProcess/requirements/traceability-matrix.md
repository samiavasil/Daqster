...
| `REQ-SW-PL-019` | Video Transform Node — Configurable Operations | ACTIVE (impl done, unit tests deferred by user decision) | `REQ-SW-PL-014` | `REQ-SW-PL-018` | `e574c63` (feat) | `src/plugins/demo_nodeditor_nodes/Sources/Video/` (VideoTransformNode, VideoTransformOps.{h,cpp}, OpenCVTransforms.cpp conditional) + `DemoNodeEditorNodesObject.cpp` (registerNodes) + `CMakeLists.txt` (OpenCV auto-detect) | Qt5/Qt6 builds PASS with and without OpenCV (DAQSTER_USE_OPENCV ON/OFF; OpenCV 4.6.0 dev) + app smoke (no crash); unit tests deferred by user decision |
