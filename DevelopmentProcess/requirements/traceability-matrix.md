...
| `REQ-SW-PL-020` | Zero-Copy Video Frame Transport & GPU Display | ACTIVE | `REQ-SW-PL-018` | `REQ-SW-PL-018`, `REQ-SW-PL-013` | — | `src/plugins/common/NodeDataTypes/VideoFrameData.h`, `src/plugins/demo_nodeditor_nodes/Sources/Video/` (VideoOutputNode dual-input + QVideoWidget GPU display, source nodes dual-port emit, VideoCompat.h present helpers) + `CMakeLists.txt` (MultimediaWidgets) + `docs/plugins/demo_nodeditor_nodes/README.md` (Windows specifics) | Qt5/Qt6 builds + existing test regression (no test modifications) + app smoke (no crash) |
