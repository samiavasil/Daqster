...
| `REQ-SW-PL-018` | Video Source & Processing Nodes (Demo Nodes) | ACTIVE (impl done, unit tests deferred by user decision) | `REQ-SW-PL-014` | `REQ-SW-PL-013` | `e5b7309`, `ef1b66c` | `src/plugins/demo_nodeditor_nodes/Sources/Video/` (VideoCompat.h, CameraSourceNode, VideoFileSourceNode, StreamSourceNode, VideoOutputNode, VideoModifierNode) + `DemoNodeEditorNodesObject.cpp` (registerNodes) | Qt5/Qt6 builds + app smoke (no crash); unit tests deferred by user decision |
| `REQ-SW-PL-019` | Video Transform Node — Configurable Operations | ACTIVE (created before implementation; ACs pending) | `REQ-SW-PL-014` | `REQ-SW-PL-018` | — (to be filled on implementation) | `src/plugins/demo_nodeditor_nodes/Sources/Video/` (VideoTransformNode, op engine, OpenCVTransforms.cpp conditional) + `DemoNodeEditorNodesObject.cpp` (registerNodes) + `CMakeLists.txt` (OpenCV auto-detect) | Qt5/Qt6 builds (8 base ops); OpenCV ops verified when dev headers present; unit tests deferred by user decision |
