...
- [x] 1. Input: конфигурируеми ДВЕ директории с изисквания (public + private), всяка парсвана от `RequirementsParser`.
- [x] 2. Merged model: обединен модел/дърво, всяко изискване носи repo идентификатор (колона/група "Repo" в UI).
- [x] 3. Cross-repo референции (`Родител:`/`Зависи от:` към другото repo) се валидират коректно — не се отчитат като dangling.
- [x] 4. Traceability matrix обхваща и двете дървета (REQ-SW-PL/FW/APP/BLD-* + REQ-AI/PLG/SEC).
- [x] 5. Няма регресия на съществуващото single-directory поведение (Phase 1&2 функционалност).
...
(комити `0429ea8`, `9d90fff`). Всички AC 1-6 са mark-нати `[x]` — AC 1-5
(две директории, merged model + Repo, cross-repo валидация, matrix върху двете
дървета, без регресия) са имплементирани и покрити от `TestMerge` + `TestValidator`
unit тестовете, AC 6 (Tests) — от верификацията (Qt5 + Qt6 builds + unit тестове),
записана в Проследимост. Статус → **DONE**; файлът остава в `active/`
